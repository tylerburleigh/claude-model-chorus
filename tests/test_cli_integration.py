...
Tests the complete CLI interface including argument, ideate, and research commands
...
class TestResearchCommand:
    """Test suite for 'research' CLI command."""

    @patch('modelchorus.cli.main.get_provider_by_name')
    @patch('modelchorus.cli.main.ResearchWorkflow')
    def test_research_basic_invocation(
        self,
        mock_workflow_class,
        mock_get_provider,
        cli_runner,
        ...
        mock_workflow = MagicMock()
        mock_workflow.run = AsyncMock(return_value=mock_workflow_result)
        mock_workflow.ingest_source = MagicMock()
        mock_workflow_class.return_value = mock_workflow
        ...
        assert result.exit_code == 0
        assert "Conducting systematic research" in result.stdout
        assert "Test synthesis result" in result.stdout

    @patch('modelchorus.cli.main.get_provider_by_name')
    @patch('modelchorus.cli.main.ResearchWorkflow')
    def test_research_with_citation_style(
        self,
        mock_workflow_class,
        mock_get_provider,
        cli_runner,
        ...
        for style in ['informal', 'academic', 'technical']:
            result = cli_runner.invoke(
                app,
                ["research", "Test query", "--citation-style", style]
            )

            assert result.exit_code == 0
            call_kwargs = mock_workflow.run.call_args[1]
            assert call_kwargs['citation_style'] == style

    @patch('modelchorus.cli.main.get_provider_by_name')
    def test_research_invalid_citation_style(
        self,
        mock_get_provider,
        cli_runner,
        mock_provider
    ):
        """Test research with invalid citation style."""
        mock_get_provider.return_value = mock_provider

        result = cli_runner.invoke(
            app,
            ["research", "Test", "--citation-style", "invalid"]
        )

        assert result.exit_code == 1
        assert "Invalid citation style" in result.stdout

    @patch('modelchorus.cli.main.get_provider_by_name')
    @patch('modelchorus.cli.main.ResearchWorkflow')
    def test_research_with_depth(
        self,
        mock_workflow_class,
        mock_get_provider,
        cli_runner,
        ...
        for depth in ['shallow', 'moderate', 'thorough', 'comprehensive']:
            result = cli_runner.invoke(
                app,
                ["research", "Test query", "--depth", depth]
            )

            assert result.exit_code == 0
            call_kwargs = mock_workflow.run.call_args[1]
            assert call_kwargs['research_depth'] == depth
    ...
    def test_research_invalid_depth(
        ...
        """Test research with invalid depth level."""
        mock_get_provider.return_value = mock_provider

        result = cli_runner.invoke(
            app,
            ["research", "Test", "--depth", "invalid"]
        )

        assert result.exit_code == 1
        assert "Invalid research depth" in result.stdout

    @patch('modelchorus.cli.main.get_provider_by_name')
    @patch('modelchorus.cli.main.ResearchWorkflow')
    @patch('builtins.open', new_callable=mock_open, read_data="Source content")
    def test_research_with_source_files(
        self,
        mock_file,
        mock_workflow_class,
        mock_get_provider,
        ...
        """Test research with source files."""
        mock_get_provider.return_value = mock_provider
        mock_workflow = MagicMock()
        mock_workflow.run = AsyncMock(return_value=mock_workflow_result)
        mock_workflow.ingest_source = MagicMock()
        ...
        result = cli_runner.invoke(
            app,
            ["research", "Analyze feedback", "--file", str(temp_test_file)]
        )

        assert result.exit_code == 0
        # Verify ingest_source was called
        mock_workflow.ingest_source.assert_called_once()
        call_args = mock_workflow.ingest_source.call_args[1]
        assert call_args['source_type'] == 'document'
        assert call_args['credibility'] == 'high'

    @patch('modelchorus.cli.main.get_provider_by_name')
    @patch('modelchorus.cli.main.ResearchWorkflow')
    def test_research_with_multiple_files(
        self,
        mock_workflow_class,
        mock_get_provider,
        cli_runner,
        ...
        """Test research with multiple source files."""
        # Create multiple test files
        file1 = tmp_path / "source1.txt"
        file2 = tmp_path / "source2.txt"
        file1.write_text("Source 1 content")
        file2.write_text("Source 2 content")
        ...
        assert mock_workflow.ingest_source.call_count == 2

    @patch('modelchorus.cli.main.get_provider_by_name')
    def test_research_with_nonexistent_file(
        self,
        mock_get_provider,
        cli_runner,
        mock_provider
    ):
        """Test research with nonexistent source file."""
        mock_get_provider.return_value = mock_provider

        result = cli_runner.invoke(
            app,
            ["research", "Test", "--file", "/nonexistent/source.txt"]
        )
    ...
    def test_research_with_output(
        ...
        result = cli_runner.invoke(
            app,
            ["research", "Test query", "--output", str(temp_output_file)]
        )
        ...
        assert output_data['success'] is True
        assert output_data['research_question'] == "Test query"
        assert 'dossier' in output_data
        assert 'findings' in output_data
        assert 'citation_style' in output_data
        assert 'research_depth' in output_data
