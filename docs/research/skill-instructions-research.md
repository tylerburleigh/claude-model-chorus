...
When AI agents use these workflows via MCP tools (Zen MCP server), they use Python API calls instead of CLI commands. The MCP server translates tool calls to workflow execution:

```python
# MCP Tool Call Pattern (what AI sees)
mcp__zen__chat(
    prompt="User query here",
    model="claude",  # Maps to provider parameter
    continuation_id="thread-123",  # Optional threading
    absolute_file_paths=["/path/to/file"],  # Optional context
    temperature=0.7,  # Optional creativity control
    working_directory_absolute_path="/path/to/workdir"
)

# Internal Translation to Workflow
workflow = ChatWorkflow(provider=provider_instance, memory=memory)
...
**Key MCP Integration Notes:**

1. **Model vs Provider:** MCP tools use `model` parameter, internally maps to provider selection
2. **File Paths:** MCP requires absolute paths in `absolute_file_paths` list
3. **Working Directory:** MCP tools accept `working_directory_absolute_path` for context
4. **Threading:** Uses same `continuation_id` pattern as CLI
5. **No CLI Flags:** MCP uses keyword arguments (no short flags like `-p`, `-f`)
