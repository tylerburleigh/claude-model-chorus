...
def test_review_json_exists(review_json_path):
    ...
    assert review_json_path.exists(), f"Review JSON file not found at {review_json_path}"
...
def test_review_json_has_required_fields(review_json_data):
    ...
    required_fields = ["overall_score", "recommendation", "dimension_scores", "issues", "strengths"]
...
def test_strengths_structure(review_json_data):
    ...
    for strength in strengths:
        ...
        assert isinstance(strength["description"], str), "Strength description should be a string"
        assert len(strength["description"]) > 0, "Strength description should not be empty"
